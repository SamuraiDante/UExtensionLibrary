...
{
	/// <summary>
	/// <see langword="static"/> class containing extentions for enumerables
	/// </summary>
	public static class Enumerables
	{
		...
		public static void Append<T, T2>(this Dictionary<T, T2> dicCurrent, Dictionary<T, T2> dicToAppend, bool blnOverwriteExisting = false)
		{
			...
			{
				if(dicCurrent.ContainsKey(Key) == false)
					dicCurrent.Add(Key, dicToAppend[Key]);
				else
				{
					if(blnOverwriteExisting == true)
						dicCurrent[Key] = dicToAppend[Key];
				}
			}
		}
